            ...
            <div class="comprehensive">
                <div class="title">
                    <span class="comprehensiveCon">
                        <i v-bind:class="[ !isActive ? 'dn' : ' iconfont chooseCon']" v-on:click="choose()">&#xe636;</i>
                        <i v-bind:class="[ !isActive ? 'iconfont fontColor' : 'dn']" v-on:click="choose()">&#xe606;</i>
                        <span class="see">只看活动帖</span>
                    </span>
                </div>
                <ul class="actInfo" v-infinite-scroll="loadMoreModule" infinite-scroll-disabled="isLoading" infinite-scroll-distance="10">
    ...
    export default {
        ...
        methods: {
            ...
            goDetail: function (index) {
                ...
                this.$router.push('/details/' + this.$route.params.id + '/' + detailsId + '/')
            },
            getBarList() {
                this.$http.post(this.viewList, {categoryid:this.$route.params.id, page:this.page}).then((response) => {
...
.communityNav .title { display: inline-block; width: 80%; height: 0.48rem; font-size: 0.2rem; text-align: center; color: #fff; }
...
.communityPage .area .gamePic { display: inline-block; width: 100%; }
/* 置顶 */
.stick { display: block; width: 100%; margin-top: 10px; border-top: 8px solid #C0C0C0; }
/* 官方活动 */
.officialAct { display: block; width: 100%; height: 0.5rem; line-height: 0.5rem; margin-left: 0.1rem; overflow: hidden; }
.officialAct .iconOff { float: left; display: inline-block; width: 0.4rem; height: 0.4rem; border-radius: 0.4rem; margin-top: 0.05rem; background: url("../../assets/images/forum/community/stick/icon-sec.png") center center no-repeat; background-size: contain; }
...
/* 活动 */
.act { display: block; width: 100%; height: 0.5rem; line-height: 0.5rem; margin-left: 0.1rem; overflow: hidden; }
.act .icon { float: left; display: inline-block; width: 0.4rem; height: 0.4rem; border-radius: 0.4rem; margin-top: 0.05rem; background: url("../../assets/images/forum/community/stick/icon-trd.png") center center no-repeat; background-size: contain; }
...
.act .con { display: inline-block; float: right; width: 70%; font-size: 0.13rem; }
/* 综合 */
.comprehensive { display: block; width: 100%; }
.comprehensive .title { height: 0.5rem; line-height: 0.5rem; font-size: 0.16rem; }
.comprehensive .title .comprehensiveCon { display: inline-block; width: 100%;   }
.fontColor { color: green; overflow: hidden; float:left; margin-left: 0.1rem; }
.chooseCon { overflow: hidden; float:left; margin-left: 0.1rem; }
.see { display: inline-block; margin-left: 0.1rem; line-height: 0.43rem; }
/* 活动信息 */
.actInfo { display: block; width: 100%; }
.actInfo .actInfoFir { display: block; width: 93%; padding: 0.1rem 0.1rem; margin-top: 0.2rem; border-bottom: 1px solid #f1f1f1; }
...
.labelFir .act { display: inline-block; width: 0.6rem; height: 0.2rem; margin-left: 0.1rem; margin-top: 0.15rem; background: red; line-height: 0.2rem; text-align: center; font-size: 0.12rem; color: #fff; }
.labelFir .actTitle { display: inline-block; float: right; width: 76%; font-size: 0.16rem; letter-spacing: 0.01rem; }
...
.labelSec .Con { display: inline-block; width: 98%; margin-left: 2%; font-size: 0.14rem; line-height: 0.22rem; letter-spacing: 0.005rem; }
/* 帖子图片 */
.photo { display: block; width: 100%; }
.photo .photoInfoLong { display: inline-block; overflow: hidden; width: 100%; height: 1.8rem; margin-top: 0.1rem; line-height: 2rem; text-align: center; border-radius: 5px; }
