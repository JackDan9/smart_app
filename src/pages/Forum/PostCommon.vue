<template>
    <div>
        ...
        <div class="commonPage">
            ...
            <div class="title">
                <textarea type="text" placeholder="有个高大上的标题才会火..." name="text"
                    ref="commonTitle"
                    v-bind:value="commonTitle"
                    id="commonTitle"></textarea>
            </div>
            <div class="content">
                <textarea type="text" placeholder="让世界看到你的Idea..." name="text"
                    ref="commonContent"
                    v-bind:value="commonContent"
                    id="commonContent"></textarea>
            </div>
            <div class="text">
                <div id="app">
                    ...
                    <input type="hidden" id="commonPictures" name="commonPictures" value="commonPictures" ref="commonPictures" />
    ...
    export default {
        ...
        methods: {
            ...
            updateCommonCon: function(commonTitle, commonContent) {
                this.$http.post(this.barUpdateUrl, {id: this.$route.params.id, title: commonTitle, content: commonContent}).then((response) => {
                    const ret = JSON.parse(response.data || "[]")
                    if(ret && ret.code === 0) {
                        Toast('发帖信息更新成功!')
                    } else {
            ...
            sendUpdateCommonCon: function(commonTitle, commonContent) {
                this.$http.post(this.barUpdateUrl, {id: this.$route.params.id, uid: this.uid, username: this.username, title: commonTitle, content: commonContent}).then((response) => {
                    const ret = JSON.parse(response.data || "[]")
                    if(ret && ret.code === 0) {
                        Toast('发帖信息更新成功!')
            ...
            categoryGame: function() {
                this.categoryId = 1
                this.$http.post(this.barUpdateUrl, {id: this.$route.params.id, categoryid: this.categoryId, uid: this.uid}).then((response) => {
                    const ret = JSON.parse(response.data || "[]")
                    if(ret && ret.code === 0) {
                        Toast('分类添加成功')
                        let id = ret.id
...
.commonNav { position: fixed; left: 0; right: 0; z-index: 503; height: 0.5rem; line-height: 0.5rem; width: 100%; background: #000000;}
.commonNav .back { display: inline-block; width: 0.2rem; height: 0.2rem; background: url(../../assets/images/forum/community/icon-back.png) center center no-repeat; background-size: contain; margin-top: 0.14rem; margin-left: 0.05rem;}
.commonNav .title { display: inline-block; width: 73%; height: 0.48rem; font-size: 0.2rem; text-align: center; color: #fff; }
.commonNav .send { display: inline-block; float: right; width: 0.45rem; height: 0.25rem; margin-right: 0.09rem; margin-top: 0.13rem; border: 1px solid #C0C0C0; border-radius: 5px; color: #fff; text-align: center; line-height: 0.26rem; }
/* 分类选择 */
.commonPage { width: 100%; height: 91.2%; position: absolute; top: 8.8%; }
.commonPage .typeList { display: block; height: 0.5rem; line-height: 0.5rem; vertical-align: middle; font-size: 0.15rem; }
.commonPage .typeList .left { display: inline-block; }
.typeIcon { display: inline-block; float: left; width: 0.2rem; height: 0.2rem; margin-top: 0.135rem; margin-left: 0.1rem; margin-right: 0.1rem; background: url("../../assets/images/forum/postcommon/types.png") center center no-repeat; background-size: contain; }
.commonPage .typeList .right { display: inline-block; float: right; margin-right: 0.1rem; }
.goIcon { display: inline-block; float: right; width: 0.2rem; height: 0.2rem; margin-top: 0.135rem; margin-left: 0.1rem; background: url("../../assets/images/forum/postcommon/go.png") center center no-repeat; background-size: contain; }
/* 标题 */
.commonPage .title { display: block; width: 100%; font-size: 0.18rem; }
.title textarea { border: none; outline: none; width: 90%; padding: 5%; }
.commonPage .content { display: block; width: 100%; font-size: 0.16rem; margin-top: 0.1rem; }
...
/* 分类选择框 */
.exist { display: block; position: absolute; bottom: 0; left: 0; right: 0; width: 100%; border-radius: 4px; background: #2d2d2d; color: #fff; }
