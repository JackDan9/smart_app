<template>
    <div>
        <div class="navTop">
            ...
            <span class="sendActPlace">活动地点</span>
            <i class="sendIcon" v-on:click="sendCon()">发送</i>
        </div>
        <div class="comment">
            <textarea type="text" placeholder="请填写您的活动地点..." name="text"
                ref="placeTitle"
                value=""
                id="placeTitle"></textarea>
        </div>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            ...
            sendCon: function() {
                let placeTitle = this.$refs.placeTitle.value
                let id = this.$route.params.id
                this.updateActPlace(id, placeTitle)
            },
            updateActPlace(id, placeTitle) {
                this.$http.post(this.updateBarUrl, {id: id, address: placeTitle}).then((response) => {
...
.navTop .sendActPlace { display: inline-block; width: 70%; text-align: center; font-size: 0.18rem; color: #fff; }
