<template>
  <div>
    ...
    <div class="ask pt45">
      <div class="text">
        <textarea name="contents" id="contents" cols="30" rows="12" placeholder="您的只言片语,[智慧信阳]随时聆听,因您而美好!~我们会及时给您回复。" ref="contents"></textarea>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getComplain(contents) {
     this.$http.post(this.govUrl,{contents:contents} ).then( (response) => {
       const ret = JSON.parse(response.data || "[]")
       if(ret && ret.code === 0){
         Toast('反馈成功,请耐心等待回复!')
...
<style scoped>
.navTop .publish{ position:absolute; right:0; top:0; height:0.45rem; padding-right:0.15rem; color:#ffffff; font-size:0.15rem; line-height:0.45rem;}
