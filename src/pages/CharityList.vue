        ...
        <template v-for="charity in charitys">
          <li>
            <a v-on:click="changeClick(charity.id)">
              <div class="pic">
                <img v-if="charity['picture']" :src="fullUrl(charity['picture'])+'!640.398'"  class="w100" alt=""/>
                <div class="operation">
                  <div class="btn"></div>
                  <div class="num"><span>{{charity['love']}}</span>份爱心</div>
                </div>
              </div>
              <div class="info">
                <div class="name">{{charity['name']}}</div>
                <div class="num">已投 <span>{{charity['vote']}}</span> 票</div>
              </div>
            </a>
          </li>
        </template>
...
export default {
  ...
  methods: {
   ...
   changeClick:function(id){
    ...
    getData( callback ) {
     this.$http.post(this.viewListUrl,{page:this.page} ).then( (response) => {
       const ret = JSON.parse(response.data || "[]")
       if(ret && ret.code === 0){
         this.page = ret['data']['page'];
         var total_love_str = '已有';
         if(ret['data']['total_love'] > 0){
            for (var i = 0; i < ret['data']['total_love'].toString().length; i++) {
                 total_love_str += '<span>'+ret['data']['total_love'].toString().charAt(i)+'</span>'
             }
         }else{
...
*/
.navTop a{ position:absolute; right:0.2rem; top:0; color:#f87f19; }
/*
...
*/
.welfare-list{ overflow:hidden; padding-top: 0.45rem; padding-bottom:0.2rem;}
.welfare-list li{ position:relative; border-bottom:#e5e5e5 1px solid; overflow:hidden; margin-bottom:0.15rem; }
.welfare-list li .pic{ overflow:hidden; position:relative;}
.welfare-list li .operation{ position:absolute; left:0.2rem; bottom:0.3rem;}
.welfare-list li .operation .btn{ width:0.73rem; height:0.24rem; background:url("../assets/images/bnt-vote.png") center center no-repeat;background-size:contain;}
.welfare-list li .operation .num{ margin-top:0.1rem; font-size:0.13rem; color:#f87f19;}
.welfare-list li .operation .num span{ font-size:0.23rem; }
.welfare-list li  .info{ padding:0.1rem 0.15rem; overflow:hidden; color:#333333; font-size:0.16rem; }
.welfare-list li .info .name{ float:left;}
.welfare-list li .info .num{ float:right; color:#a7a7a7;}
.welfare-list li .info .num span{ color:#ff0101 }
.welfare-list .review{ background:#dddddd; color:#6d6d6d; height:0.45rem; line-height:0.44rem; text-align:center; font-size:0.16rem; }
</style>
