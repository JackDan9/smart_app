<template>
  <div>
    <div class="navTop">
      ...
      <span class="right">
        <i
          v-bind:class="[company['isLike'] ? 'iconfont Liked' : 'iconfont noLiked']"
          v-on:click="addLike(company['id'],company['isLike'])"
          v-bind:id="company['id']"
        >&#xe607;</i>
        <i
          class="iconfont share"
          v-on:click="shareLink(company.name,fullUrl(company.picture))"
        >&#xe60f;</i>
      </span>
    </div>
    ...
    <div class="product-tc">
      <div class="info">
        ...
        <p class="p3">
          <span>营业时间</span>
          <em>{{company['opentime']}}</em>
        </p>
        <p class="p3" v-if="company['sendtime']">
          <span>配送时间</span>
          <em>{{company['sendtime']}}</em>
        </p>
        <a v-on:click="phoneClick(company.mobile)">
          <div class="bot tel">
            <a type="tel">
              {{company['contact']}}：{{company['mobile']}}
              <i class="tel-ico"></i>
            </a>
          </div>
        </a>
        <div
          v-bind:class="[(company.lat && company.lng) ? 'bot addr' : 'addr']"
          v-on:click="mapLink(company.lat,company.lng,company.name,company.address)"
        >
      ...
      <div class="gray-line"></div>
      <div class="w-tab">
        <li v-bind:class="[isClass ? 'current' : '']" v-on:click="addChange2('con1')">
          <span>商品</span>
        </li>
        <li v-bind:class="[isClass ? '' : 'current']" v-on:click="addChange2('con2')">
          <span>评价</span>
        </li>
      </div>
            ...
            <template v-for="category in categorys">
              ...
              >
                ...
                <span
                  class="num"
                  :id="category.id"
                  v-bind:class="[category.category_num >0 ? '' : 'dn']"
                >{{category['category_num']}}</span>
              </li>
            </template>
          </ul>
        </div>
              ...
              <template v-for="product in category['product']">
                <li>
                  <router-link :to="{path:'/product/'+product['id'] ,query:{tab:'hide'}}">
                    <div class="img">
                      <img
                        ...
                        :src="fullUrl(product['picture'])+'!114.114'"
                        class="w101"
                      />
                    </div>
                  </router-link>
                  <div class="det">
                    ...
                    <div
                      class="p3"
                    >月售{{product['salenum']}}份 评分{{product['commentstar']}} 库存{{product['stock']}}</div>
                    <div class="p4">
                      <div class="fl">
                        <small>￥</small>
                        <em>{{product['price']}}</em>
                      ...
                      <div class="fr">
                        <i
                          class="iconfont sub"
                          v-on:click="addNum('sub',product.id,category.id,product.price,product.collect_num,category.category_num)"
                        >&#xe621;</i>
                        <span class="num" :id="product.id">{{product['collect_num']}}</span>
                        <i
                          class="iconfont add"
                          v-on:click="addNum('add',product.id,category.id,product.price,product.collect_num,category.category_num)"
                        >&#xe610;</i>
                      </div>
                    </div>
                  </div>
          ...
          <div class="right">
            <div class="item">
              ...
              <div class="redstars">
                ...
                <span v-bind:class="[company['star_x'] ? 'rw'+company['star_x'] : 'rw5']">
          ...
          <ul>
            <template v-for="(comment, index) in comments">
              <li :key="index">
                <div class="img">
                  <img
                    v-bind:src="[comment['picture']?fullUrl(comment['picture']):defaultPicUrl()]"
                    class="w100"
                  />
                </div>
                <div class="det">
                  ...
                  <div class="p2">
                    <div class="redstars fl">
                      <!--  20% 为1颗星，40%为2颗星，以此类推  -->
                      <span v-bind:class="[comment['star'] ? 'rw'+comment['star'] : 'rw5']">
                        <img src="~@/assets/images/star-red2.png" alt />
                      </span>
                    </div>
                  ...
                  <div class="p3">
                    <div class="digg"></div>
                    <div class="box">
                      <template v-for="(product_name, index) in comment['product']">
                        <span :key="index">{{product_name}}</span>
                      </template>
                    </div>
                  </div>
                </div>
...
export default {
  ...
  methods: {
      ...
      if (this.categorys && this.categorys.length) {
        for (var i = 0; i < this.categorys.length; i++) {
          if (this.categorys[i]["id"] == categoryid) {
            ...
            if (
              this.categorys[i]["product"] &&
              this.categorys[i]["product"].length
            ) {
    ...
    getCollect(itemid, num) {
      this.$http
        .post(this.collectUrl, { itemid: itemid, num: num, t: 1 })
        .then(response => {
          const ret = JSON.parse(response.data || "[]");
          if (ret && ret.code === 0) {
          } else {
    ...
    getLike(linkid, updown) {
      this.$http
        .post(this.likeUrl, { linkid: linkid, updown: updown, type: 8 })
        .then(
          response => {
            const ret = JSON.parse(response.data || "[]");
            if (ret && ret.code === 0) {
...
.product-tc .info .p1 span {
  vertical-align: middle;
}
.product-tc .info .p1 img {
  vertical-align: middle;
  width: 0.22rem;
}
.product-tc .info .p2 {
...
.product-tc .info .p3 span {
  color: #646464;
  margin-right: 0.05rem;
}
.product-tc .info .p3 em {
  color: #acacac;
}
.product-tc .info .bot {
  text-align: left;
...
.product-tc .info .addr {
  text-align: left;
}
.product-tc .info .addr span {
  display: inline-block;
  width: 80%;
  line-height: 0.22rem;
  color: #9c9c9c;
...
.product-tc .dis-list li {
  height: 0.3rem;
  line-height: 0.3rem;
  color: #000000;
  font-size: 0.13rem;
}
.product-tc .dis-list .discountIcon {
  overflow: hidden;
  float: left;
  color: #ff4b0f;
}
.product-tc .dis-list li .discountMoney {
  display: inline-block;
  width: 72%;
  margin-left: 0.1rem;
}
.product-tc .dis-list li .discountNum {
  color: #898989;
}
/*
...
.product-tc .w-tab li span {
  display: inline-block;
  margin-top: 0.07rem;
}
.product-tc .w-tab li.current {
  color: #fc3756;
}
.product-tc .w-tab li.current span {
  border-bottom: #fc3756 3px solid;
}
.product-tc .pro-tab-con {
  overflow: hidden;
  font-size: 0.14rem;
  height: 3.65rem;
  box-sizing: border-box;
}
.product-tc .pro-tab-con .left {
  position: relative;
  width: 0.8rem;
  float: left;
  height: 100%;
...
.product-tc .pro-tab-con .left .tab {
  text-align: center;
  background: #f8f8f8;
...
.product-tc .pro-tab-con .left .tab .hot {
  color: #ff4b0f;
  font-size: 0.12rem;
}
/*
...
*/
.product-tc .pro-tab-con .left .tab span {
  vertical-align: middle;
  margin-left: 0.02rem;
}
.product-tc .pro-tab-con .left li {
  color: #666666;
  position: relative;
...
.product-tc .pro-tab-con .left li .num {
  position: absolute;
  right: 0.1rem;
  top: 0.1rem;
  padding: 0 0.05rem;
...
.product-tc .pro-tab-con .con .t1 {
  padding: 0.1rem;
  overflow: hidden;
  color: #666666;
  background: #f8f8f8;
}
.product-tc .pro-tab-con .con .t2 {
  color: #f69826;
  font-size: 0.12rem;
...
.product-tc .pro-tab-con .con li {
  padding: 0.15rem 0.1rem;
  overflow: hidden;
  border-bottom: #eeeeee 1px solid;
}
.product-tc .pro-tab-con .con li .img {
  width: 0.6rem;
  height: 0.6rem;
  float: left;
  overflow: hidden;
...
.product-tc .pro-tab-con .con li .w101 {
  width: 100%;
  height: 100%;
}
.product-tc .pro-tab-con .con li .p1 {
  font-size: 0.15rem;
  color: #333333;
}
.product-tc .pro-tab-con .con li .p2 {
  font-size: 0.12rem;
  color: #b0b0b0;
  margin-top: 0.01rem;
}
.product-tc .pro-tab-con .con li .p3 {
  font-size: 0.12rem;
  color: #666666;
  margin-top: 0.01rem;
}
.product-tc .pro-tab-con .con li .det {
  margin-left: 0.7rem;
  line-height: 0.2rem;
}
.product-tc .pro-tab-con .con li .p4 {
  color: #ff6000;
  margin-top: 0.05rem;
}
.product-tc .pro-tab-con .con li .p4 .fl em {
  font-size: 0.18rem;
  font-weight: bold;
}
.product-tc .pro-tab-con .con li .p4 .fl span {
  font-size: 0.1rem;
  vertical-align: 0.04rem;
  color: #999999;
  text-decoration: line-through;
}
.product-tc .pro-tab-con .con li .p4 i {
  display: inline-block;
  width: 0.18rem;
  height: 0.18rem;
}
...
*/
.product-tc .pro-tab-con .con li .p4 .add {
  float: right;
  color: #ff4b0f;
}
.product-tc .pro-tab-con .con li .p4 .num {
  font-size: 0.2rem;
  text-align: center;
}
/*
...
*/
.product-tc .pro-tab-con .con li .p4 .sub {
  color: #ff4b0f;
  font-size: 0.2rem;
}
.pro-bot {
...
.pro-tab-dis .sum .fl .p1 {
  font-size: 0.23rem;
  color: #fa500f;
  margin-top: 0.03rem;
}
.pro-tab-dis .sum .fl .p2 {
  font-size: 0.12rem;
  color: #333333;
  margin-top: 0.03rem;
}
.pro-tab-dis .sum .fl .p3 {
  font-size: 0.12rem;
  color: #999999;
}
.pro-tab-dis .sum .right {
  width: 64%;
  float: left;
  overflow: hidden;
}
.pro-tab-dis .sum .right .item {
  width: 100%;
  overflow: hidden;
  text-align: center;
  margin-top: 0.05rem;
...
.pro-tab-dis .sum .right .item .name {
  margin-right: 0.05rem;
  vertical-align: middle;
  display: inline-block;
  color: #666666;
...
.pro-tab-dis .sum .right .item .num {
  margin-left: 0.05rem;
  vertical-align: middle;
  display: inline-block;
  color: #ff6432;
...
.product-tc .taglist .b1 {
  background: #fa500f;
}
.product-tc .taglist .b2 {
  background: #fc8d63;
}
.product-tc .taglist .b3 {
  background: #f5f5f5;
  color: #b4b4b4;
}
.product-tc .filter {
...
.product-tc .comm li .det {
  margin-left: 0.6rem;
  overflow: hidden;
  font-size: 0.14rem;
}
.product-tc .comm li .p1 {
  overflow: hidden;
}
.product-tc .comm li .p2 {
  overflow: hidden;
  margin-top: 0.05rem;
}
.product-tc .comm li .p2 .txt {
  padding-left: 0.1rem;
  overflow: hidden;
  color: #666666;
  font-size: 0.12rem;
}
.product-tc .comm li .p3 {
  overflow: hidden;
  margin-top: 0.05rem;
  min-height: 0.16rem;
}
.product-tc .comm li .p3 .digg {
  width: 0.15rem;
  height: 0.15rem;
  overflow: hidden;
  float: left;
...
.product-tc .comm li .p3 .box span {
  display: inline-block;
  overflow: hidden;
  width: 0.8rem;
  height: 0.24rem;
...
.product-tc .pro-tab-con .left li.current {
  background: #ffffff;
}
.product-tc .pro-tab-con .left li.current:after {
  content: " ";
  position: absolute;
  left: 0;
  top: 0;
