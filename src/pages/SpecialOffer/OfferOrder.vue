<template>
  <div>
    ...
    <div class="page-height order-page">
      <div class="nav-top">
        <ul>
          <li v-bind:class="[isProClass ? 'current' : '']" v-on:click="addChange('con1')">
            <i class="ord ord1"></i>
            <span>到店消费</span>
            <i class="rad"></i>
          </li>
          <li v-bind:class="[isProClass ? '' : 'current']" v-on:click="addChange('con2')">
            <i class="ord ord2"></i>
            <span>送货上门</span>
            <i class="rad"></i>
          </li>
      ...
      <div class="sum">
        ...
        <div class="fr">
          <span class="pp">¥{{parseFloat(totalprice)}}</span>
        </div>
      </div>
      ...
      this.$http
        ...
        .then(response => {
          ...
          if (ret && ret.code === 0) {
            ...
            this.$router.replace({
              path: "/productdetail/" + this.$route.params.id + "?tab=hide"
            });
          } else {
...
/* 产品信息 */
.order-page .productInfo {
  display: block;
  padding: 0.15rem 0.1rem;
  border-bottom: 0.1rem solid #c0c0c0;
}
.order-page .productInfo .left {
  display: inline-block;
  width: 66%;
  float: left;
  overflow: hidden;
