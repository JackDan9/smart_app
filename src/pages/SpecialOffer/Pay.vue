  ...
  <div>
    ...
    <div class="pay-page pt45">
      <div class="top-price">
        <div class="photo">
          <img
            v-bind:src="[company['picture']?fullUrl(company['picture']):defaultPicUrl()]"
            class="w101"
          />
        </div>
        <div class="right">
      ...
      <div class="pay-btn" v-on:click="getPay()">
        确认支付
        <span>¥{{payment['totalfee']}}</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getWeixinData() {
      this.$http.post(this.weixin, { id: this.$route.params.id }).then(
        response => {
          const ret = JSON.parse(response.data || "[]");
          if (ret && ret.code === 0) {
            var weChatPay = {
            ...
            if (window.postMessage)
              window.postMessage(JSON.stringify(weChatPay), "*");
            return true;
...
.pay-page .top-price .right {
  margin-left: 0.6rem;
  overflow: hidden;
}
.pay-page .top-price .right .p1 {
  font-size: 0.22rem;
  margin-top: 0.02rem;
}
.pay-page .top-price .right .p2 {
  color: #636363;
}
.pay-page .pay-list {
  overflow: hidden;
...
.pay-page .pay-list li .name {
  margin-left: 0.1rem;
  float: left;
}
.pay-page .pay-list li .chk {
  width: 0.2rem;
  height: 0.2rem;
  float: right;
  background: url("~@/assets/images/gchk1.png") center center no-repeat;
  background-size: 0.2rem 0.2rem;
}
.pay-page .pay-list li .current {
  width: 0.2rem;
  height: 0.2rem;
  float: right;
  background: url("~@/assets/images/gchk2.png") center center no-repeat;
  background-size: 0.2rem 0.2rem;
}
.pay-page .pay-btn {
  width: 93%;
