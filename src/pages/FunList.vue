      ...
      <template v-for="thing in things" v-if="!isClass">
        <div class="goods-box">
          <div class="pic">
            ...
            <div  v-bind:class="[thing['isLike'] ? 'red-heart' : 'heart']" v-on:click="addLike(thing['id'],thing['isLike'])" v-bind:id="thing['id']" style="display:none;"></div>
           </div>
           <div class="info">
   ...
   changeClick:function(id,url){
    ...
    addChange: function(con){
       ...
       if(this.isClass){
          ...
          if(this.things.length === 0) this.getThingData(()=> window.scrollTo(0,0))
          else {
              setTimeout(()=> {
                  window.scrollTo(0,this.yThing)
                  window.addEventListener('scroll', this.handleScroll);
    ...
    getData() {
     this.$http.post(this.funListUrl,{page:this.page,type:0} ).then( (response) => {
       ...
       if(ret && ret.code === 0){
         ...
         this.bufferList.setBuffer(this.key, this.funs,
            {page:this.page,types: this.types, ads:this.ads}
         )
    ...
    getThingData( callback ) {
     this.$http.post(this.thingListUrl,{page:this.pageThing} ).then( (response) => {
       const ret = JSON.parse(response.data || "[]")
       if(ret && ret.code === 0){
...
<style scoped>
.banner { position:relative; width:100%; height:auto; padding-top: 0.45rem; overflow:hidden;}
.swipe {  height:1.6rem; color: #fff; font-size: 30px; text-align: center;}
.banner img { width:100% }
.seize { width:100%; height:0.45rem; overflow:hidden; }
...
.fun-box .info .p1{ font-size: 0.2rem; color: #000000;}
.fun-box .info .p2{ margin-top:0.05rem;  }
.fun-box .info .p2 .fl { overflow: hidden; float: left; font-size: 0.17rem; color: #626262; }
.fun-box .info .p2 .fr{ color:#000000; font-size: 0.17rem; }
...
.goods-box .info .p1{ font-size: 0.2rem; color: #000000;}
.goods-box .info .p2{ font-size:0.17rem; color: #626262; margin-top:0.05rem;  }
...
.navTop .tab a{ color:#ffffff; display:inline-block;  width:0.5rem; line-height:0.42rem; font-size:0.16rem; }
.navTop .tab .current{ border-bottom: #ffffff 2px solid;}
</style>
