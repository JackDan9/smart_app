<template>
  <div>
    ...
    <div class="QA-list pt45">
      <!--提问人-->
      <div class="qtip">
        <img src="~@/assets/images/qt.png" />
      </div>
      <div class="section qt">
        <div class="item">
          <div class="photo">
            <img
              v-bind:src="[ask['u_picture']?fullUrl(ask['u_picture']):defaultPicUrl()]"
              class="w100"
            />
          </div>
        ...
        <div class="section qa" :key="index">
          <div class="item">
            ...
            <div class="info">
              ...
              <div class="text">
                <div class="text">{{comment_data['comment']}}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    addLike: function(linkid, isLike) {
      ...
      for (var i = 0, len = this.comment_list.length; i < len; i++) {
        if (this.comment_list[i]["id"] == linkid) {
          ...
          this.comment_list[i]["vote"] =
            updown === 1
              ? this.comment_list[i]["vote"] * 1 + 1
              : this.comment_list[i]["vote"] * 1 - 1;
          break;
        }
      }
...
.QA-list .item .info .top .box .likedIcon {
  color: #ff4b0f;
  font-size: 0.2rem;
}
.QA-list .item .info .top .box .noLikedIcon {
  color: #c0c0c0;
  font-size: 0.2rem;
}
.qtip {
