    ...
    <div>
        ...
        <div class="QA-list pt45">
            <div v-infinite-scroll="loadMore" infinite-scroll-disabled="isLoading" infinite-scroll-distance="10">
                <template v-for="ask_data in ask_list">
                    <div class="section">
                        <router-link :to="{ path: ask_data.id ,query:{tab:'hide'} }" append >
                            <div class="item">
                                <div class="photo">
                                    <img v-bind:src="[ask_data['u_picture']?fullUrl(ask_data['u_picture']):defaultPicUrl()]"  class="w100"/>
                                </div>
                                <div class="info">
                                    <div class="top">
                                        <div class="fl">
                        ...
                        <div class="item" v-if="ask_data['comment']">
                            ...
                            <div class="info">
                                <div class="top">
                                    ...
                                    <div class="fr">
                                        <div class="box">
                                            <i v-bind:class="[ask_data['comment']['isLike'] ? 'ans1' : 'ans0']" v-on:click="addLike(ask_data['comment']['id'],ask_data['comment']['isLike'])"></i>
    ...
    addLike: function(linkid,isLike){
      ...
      var updown = isLike == 1 ? 2 : 1
       for (var i = 0,len = this.ask_list.length; i < len; i++) {
          if(this.ask_list[i]['comment'] && this.ask_list[i]['comment']['id'] == linkid){
              this.ask_list[i]['comment']['isLike'] = updown === 1 ? 1 : 0;
              this.ask_list[i]['comment']['vote'] = updown === 1 ? this.ask_list[i]['comment']['vote']*1+1:this.ask_list[i]['comment']['vote']*1-1
              break;
            }
        }
        if(i < len){
...
.QA-list{ background:#f2f2f2; overflow:hidden; padding-bottom:0.5rem;}
...
.QA-list .item .info .top .box i{ width:0.15rem; height:0.15rem; display:inline-block; margin-top:0.03rem;
    background:url("../../assets/images/r0.png") center center no-repeat; background-size:contain;  }
...
.QA-list .item .info .text{ width:100%;overflow:hidden; margin-top:0.03rem; font-size:0.16rem; }
.QA-list .item .red{color:red}
.QA-list .item .info .top .box .ans1 { background:url("../../assets/images/an1.png") center center no-repeat; background-size:contain; }
.QA-list .item .info .top .box .ans0 { background:url("../../assets/images/an0.png") center center no-repeat; background-size:contain; }
