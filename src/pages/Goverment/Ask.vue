    ...
    <div class="ask pt45">
      <div class="text">
        <textarea 
          name="content" 
          id="content" 
          placeholder="请耐心详尽描述您的问题，向全城发出您的声音吧！" 
          ref="content">
        </textarea>
      </div>
      <div class="tips">
...
export default {
  ...
  methods: {
    ...
    getAsk(content, picture) {
      this.$http.post(this.govUrl, { content: content, picture: picture }).then(
        response => {
...
.upload-box .add-button {
  width: 0.6rem;
  height: 0.6rem;
  border: #e5e4e4 1px solid;
  text-align: center;
}
.upload-box .add-button i {
  display: inline-block;
  width: 0.2rem;
  height: 0.2rem;
  margin-top: 0.1rem;
  background: url("~@/assets/images/add-ico.png") center center no-repeat;
  background-size: contain;
}
.upload-box .add-button p {
  font-size: 0.1rem;
  color: #a5a5a5;
}
</style>
