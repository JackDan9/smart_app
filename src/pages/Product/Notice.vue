          ...
          <template v-for="(notice, index) in notices">
            <li :key="index">
              <div class="left">
                ...
                <p class="p2">
                  <span class="p22" v-show="notice['reply_user']">{{notice['reply_user']}}:</span>
                  <span class="p23">{{notice['content']}}</span>
                </p>
              </div>
              <div class="fr">
...
export default {
  ...
  methods: {
    ...
    getData(callback) {
      this.$http.post(this.viewUrl, { type: this.type, page: this.page }).then(
        response => {
          const ret = JSON.parse(response.data || "[]");
          if (ret && ret.code === 0) {
...
.addr-choice .addr-list {
  overflow: hidden;
  margin-top: 0.1rem;
  background: #ffffff;
}
.addr-choice .addr-list .w100 {
  margin-top: -20px;
}
.addr-choice .addr-list li {
  padding: 0.1rem;
  overflow: hidden;
  border-bottom: #f7f7f7 1px solid;
}
.addr-choice .addr-list li p {
  line-height: 0.24rem;
}
...
.addr-choice .addr-list li .left .p1 {
  color: #cccccc;
}
.addr-choice .addr-list li .left .p2 .p22 {
  padding-top: 8px;
  line-height: 20px;
  font-size: 16px;
  color: #a9a9a9;
}
.addr-choice .addr-list li .left .p2 .p23 {
  padding-top: 8px;
  line-height: 20px;
  font-size: 16px;
}
.addr-choice .addr-list li .fr {
  width: 20%;
}
.addr-choice .addr-list li .fr .line {
  width: 1px;
  height: 0.4rem;
  margin-top: 0.03rem;
  background: #cccccc;
}
.addr-choice .addr-list li .fr .ico {
  width: 0.2rem;
  height: 0.2rem;
  margin-top: 0.1rem;
  overflow: hidden;
