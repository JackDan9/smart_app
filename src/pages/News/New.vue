  ...
  <div>
    <div class="navTop">
      ...
      <i
        ...
        v-on:click="shareLink(detail.title,fullUrl(detail.picture))"
      >&#xe60f;</i>
    </div>
    <div class="news-page">
      <div class="top">
        ...
        <div class="source">
          <div class="logo fl">
            <img v-if="detail['picture']" :src="fullUrl(detail['picture'])+'!640.398'" class="w100" />
          </div>
          <div class="info">
      ...
      <div class="comm">
        ...
        <div class="list">
          <ul>
            <template v-for="(comment_data, index) in comments">
              <li :key="index">
                <div class="img">
                  <img
                    v-bind:src="[comment_data['u_picture']?fullUrl(comment_data['u_picture']):defaultPicUrl()]"
                    class="w100"
                  />
                </div>
                <div class="info">
                  <div class="p1">
                    ...
                    <div
                      v-on:click="addLike(comment_data['id'],comment_data['isLike'])"
                      class="sup"
                    >
        ...
        <router-link :to="{path:'/comment/'+detail['id'],query: {tab:'hide'}}">
          <div class="w-btn">查看全部{{totalNum}}条评论</div>
        </router-link>
      </div>
      <div
        class="recommend"
          ...
          <div class="ad">
            <a v-on:click="changeClick(ad['id'],ad['url'])" :id="ad['id']">
              <div class="img">
                <img
                  v-if="ad['picture']"
                  :src="fullUrl(ad['picture'])+'!640.398'"
                  class="w100"
                  alt="Smart App"
                />
              </div>
          ...
          <ul>
            <template v-for="(new_data, index) in news">
              <li v-on:click="addChange(new_data.id)" :key="index">
                <div class="img">
                  <img
                    v-if="new_data['picture']"
                    :src="fullUrl(new_data['picture'])+'!640.398'"
                    class="w100"
                  />
                </div>
                <div class="text">
...
export default {
  ...
  methods: {
    ...
    getData(callback) {
      this.$http.post(this.viewUrl, { id: this.id, page: this.page }).then(
        response => {
          const ret = JSON.parse(response.data || "[]");
          if (ret && ret.code === 0) {
...
.news-page .top .source {
  margin-top: 0.1rem;
  overflow: hidden;
}
.news-page .top .source .logo {
  width: 0.4rem;
}
.news-page .top .source .info {
  float: left;
  margin-left: 0.1rem;
}
.news-page .top .source .info span {
  color: #406599;
}
.news-page .top .source .info img {
  width: 0.8rem;
  margin-left: 0.1rem;
}
.news-page .top .source .info p {
  color: #999999;
  font-size: 0.12rem;
  margin-top: 0.05rem;
}
...
.red-box-t span {
  ...
  height: 38px;
  line-height: 38px;
  border-bottom: #f85959 2px solid;
}
.comm .list {
  overflow: hidden;
}
...
.comm .list li .info .p1 {
  overflow: hidden;
  height: 0.5rem;
  margin-top: 0.05rem;
}
.comm .list li .info .p1 .name {
  font-size: 16px;
}
.comm .list li .info .p1 .time {
  font-size: 0.12rem;
  color: #a5a5a5;
}
.comm .list li .info .p1 .sup {
  width: 0.4rem;
  height: 0.4rem;
  float: right;
...
.recommend .con li .img {
  width: 1rem;
  float: right;
}
.recommend .con li .text {
  margin-right: 1.05rem;
  font-size: 0.16rem;
}
.recommend .con li .ref {
  margin-top: 0.05rem;
  font-size: 0.12rem;
  color: #9b9b9b;
}
.recommend .con li .ref span {
