<template>
    <div>
        ...
        <div class="comment">
            <textarea type="text" placeholder="请输入您的评论内容" name="text"
                ref="comment"
                value=""
                id="comment"></textarea>
        </div>
...
export default {
    ...
    methods: {
        ...
        getComment(linkid, comment) {
            this.$http.post(this.commentUrl, {linkid: linkid, comment: comment, type: 7})
            .then( (response) => {
                const ret = JSON.parse(response.data || "[]")
                if(ret && ret.code === 0) {
                    Toast('评论成功!')
...
<style scoped>
/* 评论框头部 */
.navTop .navCon { width: 67%; }
.navTop .sendIcon { display: inline-block; width: 20%; font-size: 0.16rem; font-style: normal; color: #fff; text-align: center; }
/* 评论框 */
.comment { width: 100%; padding-top: 0.45rem; }
